...
#' module_energy_L1010.en_bal_KOR
#'
...
#' the generated outputs: \code{L1010.en_bal_EJ_R_Si_Fi_Yh_full}.
...
#' @author HCM 2026
module_energy_L1010.en_bal_KOR <- function(command, ...) {
  KOREA_REGION_ID <- 28
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L1010.en_bal_EJ_R_Si_Fi_Yh_full"))
    ...
    L101.en_bal_EJ_R_Si_Fi_Yh_full %>%
      ...
      mutate(GCAM_region_ID = KOREA_REGION_ID) %>%
      left_join(L1010.KOR_overlay_sparse, by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
    ...
    L101.en_bal_EJ_R_Si_Fi_Yh_full %>%
      ...
      bind_rows(L1010.KOR_full) %>%
      arrange(GCAM_region_ID, sector, fuel, year) ->
      L1010.en_bal_EJ_R_Si_Fi_Yh_full
    ...
    L1010.en_bal_EJ_R_Si_Fi_Yh_full %>%
      ...
      add_legacy_name("L1010.en_bal_EJ_R_Si_Fi_Yh_full") %>%
      add_precursors("energy/KOR_en_bal.csv", "L101.en_bal_EJ_R_Si_Fi_Yh_full") ->
      L1010.en_bal_EJ_R_Si_Fi_Yh_full

    return_data(L1010.en_bal_EJ_R_Si_Fi_Yh_full)
  } else {
